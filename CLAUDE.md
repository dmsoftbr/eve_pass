...
**Fases 0, 1, 2, 4 done; Fases 3 and 5 partial. All code compiles clean.** See `docs/STATUS.md` — the canonical progress doc; update it (and the PRD banner + plan roadmap) as work lands.
...
- **Fase 5 (partial)** — three of four optional modules done in the core + tested: **5B** post-quantum hybrid X25519+ML-KEM-768 (`core/src/pq.rs`, versioned wrap `0x02`), **5C** Secret Key 2SKD (`keys::derive_enc_auth_with_secret`), **5D** passkeys P-256 ES256 (`core/src/passkey.rs`). **5A** browser extension (`apps/browser-extension/`, MV3 + native-messaging protocol) is a scaffold — needs the desktop host + pairing. 5B/5C/5D are validated primitives **not yet wired into the live account/collection flow** (hence `allow(dead_code)` on pq + `derive_enc_auth_with_secret`); that integration is the remaining work.
